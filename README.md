...
**数据流:** `入口 (CLI/MCP) → lib.rs 按 Channel 分发 → channel/pc.rs 读取 dmp → 提取流 → 符号预取 → http_symbol_supplier 解析堆栈 → build_report() → 文本/JSON 输出`

**扩展新渠道:** 在 `src/channel/` 下新增 `<渠道>.rs`，实现 `analyze()` 函数，无需改动其他模块。
...
| `--channel <渠道>` | 目标平台: `pc` (默认), `android`, `ios`（后两个尚未实现，预埋） |
| `-h, --help` | 显示帮助 |
